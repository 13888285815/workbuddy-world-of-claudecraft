...
describe('snapshot interpolation continuity', () => {
  function bareClient(pid: number): any {
    ...
    const blankEntity = (id: number) => ({
      id, pos: { x: 0, y: 0, z: 0 }, prevPos: { x: 0, y: 0, z: 0 },
      facing: 0, prevFacing: 0, kind: 'player', templateId: '', name: '',
    ...
    c.applySnapshot = function(snap: any): void {
      ...
      const applyWire = (w: any): any => {
        ...
        // Overhead emotes - simplified check
        e.overheadEmoteId = (typeof w.emo === 'string' && w.emo.length > 0) ? w.emo : null;
      ...
      if (s) {
        ...
        if (e) {
          seen.add(s.id);
          e.resource = s.res ?? 0; e.maxResource = s.mres ?? 100; e.resourceType = s.rtype ?? 'mana';
